#!/bin/bash
# humanizer — statusline badge script for Claude Code
# Reads the humanizer mode flag file and outputs a colored badge.
#
# Usage in ~/.claude/settings.json:
#   "statusLine": { "type": "command", "command": "bash /path/to/humanizer-statusline.sh" }

FLAG="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/.humanizer-active"
...
if [ -z "$MODE" ] || [ "$MODE" = "balanced" ]; then
  printf "${GREEN}[humanizer]${RESET}"
else
  SUFFIX=$(printf '%s' "$MODE" | tr '[:lower:]' '[:upper:]')
  printf "${GREEN}[humanizer:%s]${RESET}" "$SUFFIX"
fi
