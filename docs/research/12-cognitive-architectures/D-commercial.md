...
- **Inworld AI** — `inworld.ai`. Tagline: "The most advanced Character Engine for AI NPCs." Delivers arguably the most complete commercial approximation of a classical cognitive architecture on top of LLMs. Named subsystems include:
  - **Configurable Reasoning Module** — context-rich reasoning steps, game-state integration, "state of mind" reasoning where *internal thoughts differ from spoken dialogue*, conditional goals/behaviors.
  - **Long-Term Memory** of past interactions, player choices, traded items.
  - **Autonomous Goals & Motivations** (e.g. "Protect the tavern") that drive action and dialogue selection.
  - **Emotional Fluidity** across joy/anger/fear + nuanced states (nostalgia, envy) affecting dialogue and vocal tone in real time.
  - **Spatial Perception** so characters can comment on the player and react to nearby events.

  2026 launched the **Unreal AI Runtime** (C++ orchestration, single low-latency API across hundreds of models) plus **TTS-1.5-Max** with <200 ms latency and Unreal/Unity/Node.js SDKs. Ubisoft's NEO NPCs prototype runs on it. For a humanization project this is the single most reusable commercial blueprint for "reasoning + memory + goals + emotion + perception" packaged as a product.
...
- **Emotion productized as a dimension grid.** Hume's 48 emotions × 600+ voice descriptors × 10 voice-control dimensions is the clearest sign that affect is being commoditized into sliders.
- **Post-transformer substrate money arrives.** Extropic's Z1 chip (early 2026), AMI Labs' $1.03B seed, Symbolica's category-theoretic architecture, Sakana's evolutionary population — 2026 is the first year real capital flows toward *not-a-transformer* bets.
