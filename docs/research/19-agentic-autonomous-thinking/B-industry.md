...
3. **Reasoning and action fuse at the product level.** OpenAI's ChatGPT agent (unifying Operator + deep research), Anthropic's computer use, DeepMind's SIMA 2, and Adept's ACT-1 all converge on the same architecture: one model, one loop, fluid switching between thinking and doing.
4. **Code-as-action beats JSON-as-action.** Hugging Face, Imbue, Sakana (AI Scientist), and the Executable Code Actions line of papers all endorse writing tool calls as Python rather than JSON — a position now spreading into Claude Code and Codex harnesses.
...
- **From chat-shaped to computer-shaped.** ACT-1 (2022) predicted it; Anthropic's computer use (Oct 2024) shipped it; ChatGPT agent + Claude Code (2025) standardized it. The primary interface for an autonomous agent is now a screen/terminal, not a chat box.
- **From single-agent to *intentionally* single-agent.** Mid-2023 multi-agent hype (AutoGen, Swarm, MetaGPT) has been actively rebuked by Cognition and implicitly by Anthropic and Claude Code's design. The current consensus is: one coherent thread + subagents only for Q&A/research.
