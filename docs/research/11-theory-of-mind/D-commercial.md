...
- **Woebot** — Stanford-origin CBT chatbot. **ToM hook:** structured, decision-tree CBT modules teaching cognitive distortions, thought reframing, behavioral activation. Explicitly scripted rather than open-ended — user models are thin but the *therapeutic* mental model is deep. RCTs show depression/anxiety reductions.
...
- **Inflection Pi** — `hey.pi.ai` / `inflection.ai` — "the first emotionally intelligent AI." **ToM hook:** marketing explicitly leads with EQ as a peer of IQ. Pi runs on Inflection-2.5 (~94% of GPT-4 quality at 40% of training compute). 1M DAU / 6M MAU, 4B+ messages exchanged, 33-min average conversation length, ~60% weekly return rate — the strongest published companion-AI retention numbers. Responses described as kind, supportive, suggesting concrete coping steps.
...
- **Voice-native is the new default for ToM.** Hume EVI, Slingshot Ash, Pi, Affectiva — and the CX incumbents all heavily invest in tonality. Text-only ToM is being reframed as the low-rent tier.
- **Digital twins are eating early-stage research.** Synthetic Users, Synthetic Respondents, Ask Rally/GenPop, FishDog, Twin Persona — four+ well-funded players in under 24 months. Most explicitly position against real recruitment, not alongside it (the "invert the process, don't replace it" line from Synthetic Users is a rearguard framing).
