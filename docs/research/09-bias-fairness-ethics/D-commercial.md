...
> Research date: April 2026. Standard field template below is applied consistently so entries can be compared across rows.
...
### 12. Google SynthID + C2PA ecosystem
- **Category:** Machine-readable AI watermarking + provenance
- **Ownership:** Google DeepMind (SynthID); C2PA is a cross-industry consortium (Adobe, MSFT, Intel, BBC, etc.)
- **Positioning:** "Dual-layer authentication" combining imperceptible watermark + cryptographic metadata
- **Key features:** SynthID embeds invisible markers across text/image/audio/video; applied by default to Gemini, Imagen 3, Veo 2 outputs; 10B+ pieces of content marked as of Jan 2026; C2PA 2.0 added visible icon for browsers and media players in April 2025
- **Pricing:** Bundled in Google/Gemini/Vertex AI
...
- **Market reality:** C2PA is fragile under recompression; SynthID is robust but carries minimal provenance. Art. 50 appears to accept either — so procurement is driven by which foundation model a team uses
...
- **Key features / accuracy (indep. benchmark, 3K samples):**
  - **GPTZero:** 99.3% accuracy, 0.24% FP — strongest on GPT-5 and o3 outputs
  - **Copyleaks:** 90.7% accuracy, 5.26% FP — 30+ languages, mixed human/AI detection, AI Logic explanations
  - **Originality.ai Enterprise:** 83.0% accuracy, 4.79% FP — WordPress + Chrome integrations, plagiarism checker, predictive SEO
- **Known weakness:** **20–30% accuracy drop when AI output is lightly edited or "humanized"** — directly relevant to Category 05 (AI Text Detection & Evasion) and to Art. 50 enforceability
- **Relevance:** These are the runtime tools platforms use to *verify* disclosure claims; their failure mode on humanized output is a structural vulnerability of any disclosure regime that relies on post-hoc detection
...
4. **EU AI Act Article 50 is the dominant 2026 driver.** Enforcement date 2 Aug 2026, up to €15M / 3% turnover for transparency violations. Every vendor above has published an Art. 50 mapping; this is the first time a *humanization-specific* regulatory line (AI-disclosure at first contact, deepfake labeling, emotion-recognition notice) has commercial product gravity.
5. **Machine-readable marking is converging on dual-layer (watermark + C2PA).** Google leads; Adobe, Microsoft, Meta, OpenAI all signed on. The technical debate (watermark fragility vs. provenance richness) is essentially settled by stacking both.
...
- **Companion-AI liability is the current enforcement frontier.** Character.AI settled (Jan 2026) with Google over teen-suicide lawsuits; Kentucky AG sued (Jan 2026); Texas AG investigation (March 2026). Expected result: product-liability-style disclosure and age-gating will become mandatory faster than statutory timelines.
