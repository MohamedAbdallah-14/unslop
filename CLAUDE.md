...
These paths are authoritative. Their generated copies come from `scripts/sync-mirrors.sh`, run by `.github/workflows/sync.yml`. **Edit the SSOT only.**
...
| Python file-rewriter code | `unslop/scripts/` |
| Release history | Root `CHANGELOG.md` |

Authored sub-skills (`unslop-commit`, `unslop-review`, `unslop-help`, `unslop-reasoning`) are authoritative under `skills/<name>/SKILL.md`. `skills/unslop-file/` is generated from `unslop/`.
...
The Python package contract must pass after any package change. The full matrix also covers Node 24 hook integration, repository wiring, and offline benchmark monotonicity. LLM and real-model tests are opt-in because they need API credits or large model downloads. Use [`docs/RELEASING.md`](./docs/RELEASING.md) for the tag and publication sequence.
