...
A bundled humanizer skill that rewrites memory files (CLAUDE.md, AGENTS.md, todos, preferences, docs) to remove AI slop while preserving every code block, URL, path, and heading exactly.

## What It Do

Slash command: `/humanize <filepath>`
...
```bash
cd humanizer-humanize && python3 -m scripts --deterministic <absolute_filepath>
```
...
```bash
cd humanizer-humanize && python3 -m scripts <absolute_filepath>
```
...
`humanizer-humanize` calls a subprocess (the `claude` CLI) when `ANTHROPIC_API_KEY` is unset. Snyk may flag this as High Risk because of the `subprocess.run` + file I/O combination. See [SECURITY.md](./SECURITY.md) for the full rationale.

The deterministic mode does no subprocess and no network — use it if your security context requires that.

## Install

Bundled with the `humanizer` plugin. Skill path: `humanizer-humanize/`.

Standalone: `cd humanizer-humanize && python3 -m scripts <filepath>` (Python 3.10+).
...
## Part of the Humanizer Plugin

| Skill | What it does |
|-------|--------------|
| `humanizer` | Active humanization mode for live responses |
| `humanizer-commit` | Commit messages without AI tone |
| `humanizer-review` | Direct, human-voice PR comments |
| `humanizer-humanize` | This: rewrite memory files |
| `humanizer-help` | Quick-reference card |

## License

MIT — same as the parent humanizer plugin.
