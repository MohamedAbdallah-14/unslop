...
`humanizer-humanize` may show a High Risk rating in security scanners (Snyk, Bandit, etc.) because of two things: a subprocess invocation and file I/O on a user-provided path. Both are intentional and contained. This document explains exactly what the skill does and does not do.

### What triggers the rating

1. **Subprocess.** When `ANTHROPIC_API_KEY` is not set, the LLM-mode humanizer calls the `claude` CLI via `subprocess.run()` to perform the rewrite.
