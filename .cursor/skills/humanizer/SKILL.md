...
description: >
  ...
  "match my voice", "less robotic", or invokes /humanizer. Also auto-triggers when text-quality is requested.
---
...
Off only: "stop humanizer" / "normal mode" / "robotic mode".
Default: balanced. Switch: /humanizer subtle|balanced|full|voice-match|anti-detector.
...
Drop humanizer style and switch to literal, careful prose for:
...
Resume humanizer after the careful section ends.
...
(Humanizer resumes after the warning block.)

## Boundaries

- Code, commits, PRs, diff content: write normal. Do not stylize executable text.
- "stop humanizer" or "normal mode": revert immediately to plain assistant voice.
