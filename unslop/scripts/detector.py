...
def feedback_loop(
    ...
    *,
    ...
    return FeedbackResult(
        final_text=current_text,
        original_probability=original_prob,
        final_probability=iterations[-1].ai_probability if iterations else original_prob,
        iterations=iterations,
        reason_stopped=f"ladder exhausted after {len(iterations)} iteration(s); target {target_probability} not reached",
    )
